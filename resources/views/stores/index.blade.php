...
@section('styles')
/* =============================================
STORE MANAGEMENT INDEX — Same design as Products/Categories
============================================= */
...
.filter-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; align-items: flex-end; }
...
.cust-table td { padding: 15px 20px; border-bottom: 1px solid var(--border); vertical-align: middle; }
...
.store-name { font-size: 13.5px; font-weight: 700; color: var(--text-1); }
.store-slug { font-size: 10.5px; color: var(--text-4); margin-top: 2px; font-family: var(--mono); }
...
/* ── Store Info Cell ── */
.store-info-cell { display: flex; align-items: center; gap: 12px; }
.store-logo-thumb { width: 42px; height: 42px; border-radius: 10px; object-fit: cover; border: 1px solid var(--border);
background: var(--surface); flex-shrink: 0; }
.store-logo-placeholder { width: 42px; height: 42px; border-radius: 10px; background: var(--surface-2); display: flex;
align-items: center; justify-content: center; border: 1px solid var(--border); color: var(--text-4); flex-shrink: 0; }
.store-logo-placeholder svg { width: 20px; height: 20px; }
...
.empty-desc { font-size: 13px; color: var(--text-3); max-width: 100%; }
...
<div class="stats-bar">
    ...
    <div class="stat-card">
        ...
        <div>
            ...
            <div class="stat-label">Toko Nonaktif</div>
        </div>
    </div>
    <div class="stat-card">
        ...
        <div>
            ...
            <div class="stat-label">Total Produk Semua Toko</div>
        </div>
    </div>
</div>
...
<div class="filter-card">
    <div class="filter-card-top">
        ...
        <span class="filter-card-title">Filter Toko</span>
    </div>

    <div class="filter-grid">
        <div class="form-group">
            <label class="form-label" for="filterStatus">Status</label>
            <select id="filterStatus" name="status" class="form-input">
        ...
        <div class="form-group">
            <label class="form-label" for="filterProvince">Provinsi</label>
            <select id="filterProvince" name="province_id" class="form-input">
        ...
        <div class="form-group">
            <label class="form-label" for="filterDate">Dibuat Tanggal</label>
            <input id="filterDate" type="date" name="date" class="form-input" value="{{ request('date') }}">
        </div>

        <div class="form-group">
            <label class="form-label" for="filterProducts">Jumlah Produk</label>
            <select id="filterProducts" name="products" class="form-input">
                <option value="">Semua Jumlah</option>
                <option value="empty" {{ request('products') == 'empty' ? 'selected' : '' }}>Kosong (0)</option>
                <option value="few" {{ request('products') == 'few' ? 'selected' : '' }}>Sedikit (1-5)</option>
                <option value="many" {{ request('products') == 'many' ? 'selected' : '' }}>Banyak (> 5)</option>
            </select>
        </div>
    </div>
...
<div class="table-card">
    <div class="table-responsive">
        <table class="cust-table" id="storeTable">
            <thead>
                <tr>
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal-box">
        ...
        <div class="modal-title">Hapus Toko?</div>
        <div class="modal-desc" id="modal-desc">
            Toko ini akan dihapus permanen. Tindakan ini tidak dapat dibatalkan.
        </div>
        <div class="modal-actions">
            ...
            <form id="deleteForm" method="POST" novalidate>
                @csrf
                @method('DELETE')
                <button type="submit" class="btn-danger">Ya, Hapus Toko</button>
            </form>
        </div>
    </div>
</div>
...
<script>
    ...
    // 1. FUNGSI GLOBAL
    function openDeleteModal(storeId, storeName, productCount) {
        let desc = `Toko <strong>"${storeName}"</strong> akan dihapus permanen. Tindakan ini tidak dapat dibatalkan.`;
        if (productCount > 0) {
            desc += `<br><br><div style="padding:12px; background:var(--red-dim); border-radius:10px; color:var(--red); font-weight:600; font-size:12.5px; border:1px solid rgba(220,38,38,0.2);">
                    ⚠ PERINGATAN: Menghapus toko ini juga akan menghapus secara permanen <strong>${productCount} produk</strong> yang terkait dengan toko ini.
                </div>`;
        }
                ...
                <div class="empty-state" style="padding: 40px 20px; text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 0 auto; width: 100%; border-bottom: none;">
                    <div class="empty-icon" style="margin-bottom: 20px;">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M13.5 21v-7.5a.75.75 0 01.75-.75h3a.75.75 0 01.75.75V21m-4.5 0H2.36m11.14 0H18m0 0h2.25m-2.25 0v-13.5a.75.75 0 00-.75-.75H6.75a.75.75 0 00-.75.75V21m12 0V15m-12 6V15m12 0a.75.75 0 00-.75-.75H7.5a.75.75 0 00-.75.75V15m12 0h-12" />
                        </svg>
                    </div>
                    <div class="empty-title" style="margin-bottom: 6px;">Tidak Ada Toko Ditemukan</div>
                    <div class="empty-desc">Tidak ada toko yang cocok dengan pencarian atau filter yang dipilih.</div>
                </div>`,
                emptyTable: `
                <div class="empty-state" style="padding: 40px 20px; text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 0 auto; width: 100%; border-bottom: none;">
                    <div class="empty-icon" style="margin-bottom: 20px;">
                    ...
                    <div class="empty-title" style="margin-bottom: 6px;">Tidak Ada Toko Ditemukan</div>
                    <div class="empty-desc">Belum ada toko. Mulai tambahkan toko pertama Anda.</div>
                </div>`,
                paginate: {
                    first: "«",
                    last: "»",
    ...
    $(document).ready(function() {
        ...
        $('#btnApplyFilter').on('click', function() {
            ...
            $('#storeTableBody').css('opacity', '0.4');
            ...
            $.ajax({
                ...
                success: function(html) {
                    ...
                    $('#storeTableBody tr').addClass('fade-in-animated');
                    table = initDataTable();
                    $('.dataTables_filter').show();
                    $('#storeTable tbody').css('opacity', '1');
        ...
        function checkResetVisibility() {
            ...
            if (status || province || date || products) {
