...
@section('styles')
    ...
    .status-pill { display: inline-flex; align-items: center; font-size: 11px; font-weight: 700; padding: 4px 10px; border-radius: 6px; text-transform: uppercase; letter-spacing: 0.05em; width: fit-content; }
    ...
    <div class="page-header">
        <div class="page-header-left">
            <h1>
                ...
                Detail Kota: {{ $city->name }}
            </h1>
            <p>Informasi lengkap terkait kota atau kabupaten wilayah pengiriman ini.</p>
        </div>
        <a href="{{ route('cities.index') }}" class="btn-outline">
    ...
    <div class="detail-container">
        <div class="detail-main">
            <div class="detail-card">
                <div class="detail-card-title">
                    ...
                    Informasi Kota / Kabupaten
                </div>
                <div class="info-list">
                    <div class="info-item">
                        <div class="info-label">Nama Wilayah</div>
                        <div class="info-value {{ $city->name ? '' : 'empty' }}">{{ $city->name ?: 'Data wilayah belum lengkap.' }}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Kode Wilayah</div>
                        <div class="info-value info-value-mono {{ $city->code ? '' : 'empty' }}">{{ $city->code ?: 'Data wilayah belum lengkap.' }}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Tipe Wilayah</div>
                        <div>
                            @if($city->type === 'Kota')
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value info-value-mono {{ $city->postal_code ? '' : 'empty' }}">{{ $city->postal_code ?: '-' }}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Provinsi Induk</div>
                        <div class="info-value {{ $city->province ? '' : 'empty' }}">{{ $city->province->name ?? 'Provinsi Tidak Ditemukan' }}</div>
                    </div>
                    <div class="info-item">
        ...
        <div class="detail-sidebar">
            <div class="detail-card">
                ...
                <div class="stat-box">
                    <div class="stat-box-value">{{ $city->stores_count ?? 0 }}</div>
                    <div class="stat-box-label">{{ $city->stores_count > 0 ? 'Total Toko di Kota Ini' : 'Belum ada toko yang menggunakan kota ini.' }}</div>
                </div>
            </div>
            ...
            <div class="detail-card">
                ...
                <div class="info-list">
                    <div class="info-item">
                        ...
                        <div class="info-value">{{ $city->created_at->format('d M Y, H:i') }}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Terakhir Diperbarui</div>
                        <div class="info-value">{{ $city->updated_at->format('d M Y, H:i') }}</div>
                    </div>
                </div>
            </div>
        </div>
