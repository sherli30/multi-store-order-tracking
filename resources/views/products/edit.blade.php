...
@section('styles')
    ...
    .is-primary-border { border: 2px solid var(--accent) !important; }
    .primary-badge { position: absolute; top: 4px; left: 4px; background: var(--accent); color: #fff; font-size: 8px;
    font-weight: 800; padding: 2px 4px; border-radius: 4px; text-transform: uppercase; }
    ...
    /* ── AUDIT TRAIL ─── */
    .audit-item { display: flex; justify-content: space-between; align-items: center; padding-bottom: 12px; border-bottom:
    1px dashed var(--border); }
    .audit-item:last-child { border-bottom: none; padding-bottom: 0; }
    .audit-label { font-size: 12px; color: var(--text-3); font-weight: 600; }
    .audit-value { font-size: 12px; color: var(--text-1); font-weight: 700; }
@endsection
...
@section('content')
    ...
    <div class="page-header">
        <div class="page-header-left">
            ...
            <p>Memperbarui informasi untuk <strong>{{ $product->name }}</strong></p>
        </div>
            ...
            <div class="form-main">
                    ...
                    <div class="form-card-body">
                        ...
                        <div class="field-group">
                            ...
                            <input type="text" id="name" name="name" class="field-input"
                                placeholder="Contoh: Kopi Bubuk Arabika" value="{{ old('name', $product->name) }}" required>
                        </div>
                    </div>
                </div>
                ...
                @php
                    $hasVariants =
                        ...
                        ($product->variants->first() && $product->variants->first()->name !== 'Default');
                        ...
                        <div class="toggle-card">
                            <label class="toggle-switch">
                                <input type="checkbox" id="has_variants" name="has_variants" value="1"
                                    {{ old('has_variants', $hasVariants) ? 'checked' : '' }}>
                                <span class="toggle-slider"></span>
                            </label>
                            <div>
                                <div class="toggle-label">Gunakan Variasi Produk</div>
                                <div class="field-hint">Aktifkan untuk mengelola pilihan ukuran, warna, dll.</div>
                            </div>
                        </div>
                        ...
                        <div id="singleProductConfig">
                            <div class="field-row">
                                ...
                                <div class="field-group">
                                    ...
                                    <input type="number" name="weight" id="single_weight" class="field-input"
                                        placeholder="0" min="0"
                                        value="{{ old('weight', round($defaultVariant?->weight ?? 0)) }}">
                                </div>
                            </div>
                            <div class="field-hint" style="margin-top:12px; display:flex; align-items:center; gap:8px;">
                                ...
                                Update stok dilakukan melalui menu <strong>Manajemen Stok</strong>.
                            </div>
                        </div>
                                        ...
                                        @if (old('variants'))
                                            @foreach (old('variants') as $index => $variant)
                                                <tr>
                                                    ...
                                                    <td><input type="text" name="variants[{{ $index }}][name]" class="field-input" required value="{{ $variant['name'] ?? '' }}"></td>
                                                    <td><input type="text" name="variants[{{ $index }}][sku]" class="field-input" value="{{ $variant['sku'] ?? '' }}"></td>
                                                    ...
                                                    <td><input type="number" name="variants[{{ $index }}][stock]" class="field-input" required min="0" value="{{ $variant['stock'] ?? '' }}"></td>
                                                    <td><input type="number" name="variants[{{ $index }}][weight]" class="field-input" required min="0" value="{{ $variant['weight'] ?? '' }}"></td>
                                                    <td>
                                                ...
                                                @if ($variant->name !== 'Default')
                                                    <tr>
                                                        ...
                                                        <td><input type="text"
                                                                ...
                                                                class="field-input" value="{{ $variant->sku }}"></td>
                                                        <td><input type="text" inputmode="numeric"
                                                                name="variants[{{ $index }}][price]"
                                                                class="field-input price-input" required placeholder="0"
                                                                value="{{ (int) $variant->price }}"></td>
                                                        <td><input type="number" name="variants[{{ $index }}][stock]" class="field-input" required min="0" value="{{ $variant->stock }}"></td>
                                                        <td><input type="number"
                                                                name="variants[{{ $index }}][weight]"
                                                                class="field-input" required min="0"
                                                                value="{{ round($variant->weight) }}"></td>
                                                        <td>
                            ...
                            @else
                                @foreach ($product->descriptions as $index => $desc)
                                    <div class="toggle-card" style="flex-direction:column; align-items:stretch;">
                                        <input type="hidden" name="descriptions[{{ $index }}][id]" value="{{ $desc->id }}">
                                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                                            <input type="text" name="descriptions[{{ $index }}][title]"
                                                class="field-input" style="width:70%;" placeholder="Judul * (Cth: Keunggulan)" required value="{{ $desc->title }}">
                                            <button type="button" class="btn-remove-row"
            ...
            <div class="form-sidebar">
                ...
                <div class="form-card"
                    ...
                    <div class="form-card-body" style="padding: 16px 24px; gap: 12px;">
                        <div class="audit-item">
                            ...
                            <span class="audit-value">{{ $product->created_at->translatedFormat('d M Y, H:i') }}</span>
                        </div>
                        <div class="audit-item">
                            <span class="audit-label">Terakhir Diubah:</span>
                            <span class="audit-value">{{ $product->updated_at->translatedFormat('d M Y, H:i') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-card">
                    ...
                    <div class="form-card-body" style="gap:16px;">
                        @if ($product->images->count() > 0)
                            <div class="existing-images-grid" id="existingImagesContainer">
                                @foreach ($product->images as $img)
                                    <div class="image-thumb {{ $img->is_primary ? 'is-primary-border' : '' }}"
                                        id="img-card-{{ $img->id }}">
                                        <img src="{{ asset('storage/' . $img->image_path) }}" id="img-preview-{{ $img->id }}" onclick="openImageModal(this.src)">
                                        @if ($img->is_primary)
                                            <span class="primary-badge">Utama</span>
                                        @endif
                                        <div class="image-actions">
                                            <button type="button" class="img-action-btn btn-replace" onclick="document.getElementById('replace_image_{{ $img->id }}').click()" title="Ganti Foto">Ganti</button>
                                            <button type="button" class="img-action-btn btn-delete" onclick="deleteExistingImage({{ $img->id }})" title="Hapus Foto">Hapus</button>
                                        </div>
                                        <input type="file" name="replace_images[{{ $img->id }}]" id="replace_image_{{ $img->id }}" style="display:none;" accept="image/*" onchange="previewReplacedImage(this, {{ $img->id }})">
                                    </div>
                                @endforeach
                            </div>
                        @endif
                        ...
                        <div class="image-upload-zone" onclick="document.getElementById('images').click()">
                            ...
                            <div style="font-size:11px; font-weight:700; color:var(--text-3);">Tambah Foto Baru <span>*</span></div>
                        ...
                        <div id="imageCounter" class="field-hint" style="text-align:center; display:none;"></div>
                    </div>
                </div>
                ...
                <div class="form-card">
                    ...
                    <div class="form-card-body" style="gap:12px;">
                                ...
                                @foreach ($product->specifications as $index => $spec)
                                    <div style="display:flex; gap:8px;">
                                        <input type="text" name="specifications[{{ $index }}][name]"
                                            class="field-input" style="flex:1;" placeholder="Label *" required value="{{ $spec->name }}">
                                        <input type="text" name="specifications[{{ $index }}][value]"
                                            class="field-input" style="flex:1;" placeholder="Nilai *" required value="{{ $spec->value }}">
                                        <button type="button" class="btn-remove-row" onclick="this.parentElement.remove()"
                                            style="flex-shrink:0;"><svg viewBox="0 0 24 24" width="16" height="16"
                        ...
                        <button type="button" class="btn-add-row" onclick="addSpecificationRow()"
                            style="width:100%; justify-content:center;">Tambah Spek</button>
                    </div>
                </div>
                    ...
                    <div class="form-card-body" style="gap:12px;">
                        <div id="packingContainer" style="display:flex; flex-direction:column; gap:12px;">
                            @if (old('packing_options'))
                                @foreach (old('packing_options') as $index => $opt)
                                    <div style="display:flex; gap:8px;">
                                        <input type="hidden" name="packing_options[{{ $index }}][id]" value="{{ $opt['id'] ?? '' }}">
                                        <input type="text" name="packing_options[{{ $index }}][name]" class="field-input" style="flex:1;" placeholder="Nama (Opsional)" value="{{ $opt['name'] ?? '' }}">
                                        <input type="text" inputmode="numeric" name="packing_options[{{ $index }}][extra_price]" class="field-input price-input" style="flex:1;" placeholder="Harga (Opsional)" value="{{ $opt['extra_price'] ?? '' }}">
                                        <button type="button" class="btn-remove-row" onclick="this.parentElement.remove()" style="flex-shrink:0;">
                                            <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2">
                                                <path d="M3 6h18" />
                                                <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
                                ...
                                @endforeach
                            @else
                                @foreach ($product->packingOptions as $index => $opt)
                                    <div style="display:flex; gap:8px;">
                                        <input type="hidden" name="packing_options[{{ $index }}][id]"
                                            value="{{ $opt->id }}">
                                        <input type="text" name="packing_options[{{ $index }}][name]"
                                            class="field-input" style="flex:1;" placeholder="Nama (Opsional)" value="{{ $opt->name }}">
                                        <input type="text" inputmode="numeric" name="packing_options[{{ $index }}][extra_price]"
                                            class="field-input price-input" style="flex:1;" placeholder="Harga (Opsional)"
                                            value="{{ (int) $opt->extra_price }}">
                                        <button type="button" class="btn-remove-row" onclick="this.parentElement.remove()"
                                            style="flex-shrink:0;"><svg viewBox="0 0 24 24" width="16" height="16"
                                                fill="none" stroke="currentColor" stroke-width="2">
                                                <path d="M3 6h18" />
                        ...
                        <button type="button" class="btn-add-row" onclick="addPackingRow()"
                            style="width:100%; justify-content:center;">Tambah Packing</button>
                    </div>
                </div>
    ...
    <script>
        ...
        function previewImages(input) {
            ...
            if (input.files && input.files.length > 0) {
                ...
                counter.textContent = `${input.files.length} file baru dipilih`;
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        if (data.length > 0) {
                            ...
                            categorySelect.innerHTML =
                                '<option value="" disabled selected>Tidak ada kategori aktif untuk toko ini</option>';
                        }
                    })
                    .catch(err => {
                        console.error('Error loading categories:', err);
                        categorySelect.innerHTML =
                            '<option value="" disabled selected>Gagal memuat kategori</option>';
                    });
            }

            if (storeSelect.value) {
                loadCategories(storeSelect.value, initialCategoryId);
            }
        ...
        const inputsToToggle = ['single_price', 'single_weight'];
        ...
        let variantIndex = {{ (is_array(old('variants')) ? max(array_keys(old('variants'))) : $product->variants->count()) + 10 }};

        function addVariantRow() {
            const tbody = document.querySelector('#variantsTable tbody');
            const tr = document.createElement('tr');
            tr.innerHTML = `
            <td><input type="text" name="variants[${variantIndex}][name]" class="field-input" required placeholder="Nama Varian"></td>
        ...
        let descIndex = {{ (is_array(old('descriptions')) ? max(array_keys(old('descriptions'))) : $product->descriptions->count()) + 10 }};
        ...
        let specIndex = {{ (is_array(old('specifications')) ? max(array_keys(old('specifications'))) : $product->specifications->count()) + 10 }};
        ...
        let packingIndex = {{ (is_array(old('packing_options')) ? max(array_keys(old('packing_options'))) : $product->packingOptions->count()) + 10 }};
        ...
        function previewReplacedImage(input, imgId) {
            if (input.files && input.files[0]) {
                const reader = new FileReader();
                reader.onload = function(e) {
                    const img = document.getElementById('img-preview-' + imgId);
                    if (img) {
                        img.src = e.target.result;
                    }
                }
