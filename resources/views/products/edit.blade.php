...
@section('styles')
    ...
    /* ── AUDIT TRAIL ─── */
    .audit-item { display: flex; justify-content: space-between; align-items: center; padding-bottom: 12px; border-bottom: 1px dashed var(--border); }
                ...
                <div class="form-card">
                    ...
                    <div class="form-card-body">
                        ...
                        <div class="field-group">
                            ...
                            <input type="text" id="sku" name="sku" class="field-input {{ $errors->has('sku') ? 'is-invalid' : '' }}" placeholder="Contoh: KBA-250" value="{{ old('sku', $product->sku) }}" required>
                        ...
                        <div class="field-group">
                            ...
                            <select id="store_id" name="store_id" class="field-input {{ $errors->has('store_id') ? 'is-invalid' : '' }}" required onchange="checkStoreStatus()">
                                ...
                                @foreach($stores as $store)
                                    @if($store->is_active || $store->id == old('store_id', $product->store_id))
                                        <option value="{{ $store->id }}" data-active="{{ $store->is_active ? '1' : '0' }}" {{ old('store_id', $product->store_id) == $store->id ? 'selected' : '' }}>
                        ...
                        <div class="field-group">
                            ...
                            <select id="category_id" name="category_id" class="field-input {{ $errors->has('category_id') ? 'is-invalid' : '' }}" required onchange="checkStoreStatus()">
                                @foreach ($categories as $category)
                                    @if($category->is_active || $category->id == old('category_id', $product->category_id))
                                        <option value="{{ $category->id }}" data-active="{{ $category->is_active ? '1' : '0' }}"
                                            {{ old('category_id', $product->category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }} {{ !$category->is_active ? '(Non-aktif)' : '' }}
                                        </option>
                                    @endif
                                @endforeach
                            </select>
                ...
                <div class="form-card">
                    <div class="form-card-header">
                        <div class="form-card-header-left">
                            ...
                            <span class="form-card-header-title">Harga & Berat</span>
                        </div>
                    </div>
                    <div class="form-card-body">
                        <div class="field-row">
                            <div class="field-group">
                                ...
                                <div style="position:relative;">
                                    ...
                                    <input type="text" inputmode="numeric" name="price" id="single_price" class="field-input price-input {{ $errors->has('price') ? 'is-invalid' : '' }}"
                                        style="padding-left:42px;" placeholder="0" required
                                        value="{{ old('price', (int) ($product->price ?? 0)) }}">
                                </div>
                            ...
                            <div class="field-group">
                                ...
                                <input type="number" step="0.01" name="weight" id="single_weight" class="field-input {{ $errors->has('weight') ? 'is-invalid' : '' }}" placeholder="Contoh: 0.5 kg" min="0" required value="{{ old('weight', (float)($product->weight ?? 0)) }}">
                                @error('weight') <span class="field-hint" style="color:var(--red);">{{ $message }}</span> @enderror
                            </div>
                        </div>
                        <div class="field-hint" style="display:flex; align-items:center; gap:8px;">
                            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2">
                                <circle cx="12" cy="12" r="10" />
                                <line x1="12" y1="8" x2="12" y2="12" />
                                <line x1="12" y1="16" x2="12.01" y2="16" />
                            </svg>
                            Update stok dilakukan melalui menu <strong>Kelola Stok</strong>.
                        </div>
                    </div>
                </div>
            ...
            <div class="form-sidebar">
                        ...
                        <div class="toggle-card {{ $errors->has('is_active') ? 'is-invalid' : '' }}" id="activeToggleCard" style="{{ $errors->has('is_active') ? 'border-color: var(--red);' : '' }}">
                            <label class="toggle-switch">
                                ...
                                <input type="checkbox" name="is_active" id="productActiveInput" value="1"
                                    {{ old('is_active', $product->is_active) ? 'checked' : '' }} onchange="updateActiveStatusLabel(this)">
                                <span class="toggle-slider"></span>
                            </label>
                            <div class="toggle-label" id="productActiveLabel">{{ old('is_active', $product->is_active) ? 'Produk Aktif' : 'Produk Nonaktif' }}</div>
                        </div>
                        <div class="toggle-card {{ $errors->has('is_featured') ? 'is-invalid' : '' }}" style="{{ $errors->has('is_featured') ? 'border-color: var(--red);' : '' }}">
                            <label class="toggle-switch">
                                ...
                                <input type="checkbox" name="is_featured" id="productFeaturedInput" value="1"
                                    {{ old('is_featured', $product->is_featured) ? 'checked' : '' }} onchange="document.getElementById('productFeaturedLabel').innerText = this.checked ? 'Produk Unggulan' : 'Produk Reguler'">
                                <span class="toggle-slider"></span>
                            </label>
                            <div class="toggle-label" id="productFeaturedLabel">{{ old('is_featured', $product->is_featured) ? 'Produk Unggulan' : 'Produk Reguler' }}</div>
                        </div>
                ...
                <div class="form-card">
                    ...
                    <div class="form-card-body" style="gap:16px;">
                        ...
                        <div class="image-upload-zone" onclick="document.getElementById('images').click()">
                            ...
                            <div style="font-size:11px; font-weight:700; color:var(--text-3);">Tambah Foto Baru</div>
                ...
                <div class="form-card">
                    ...
                    <div class="form-card-body" style="gap:12px;">
                        ...
                        <button type="button" class="btn-add-row" onclick="addSpecificationRow()" style="width:100%; justify-content:center;">Tambah Spek</button>
                    </div>
                </div>
...
@push('scripts')
    <script>
        ...
        function checkStoreStatus() {
            ...
            const storeWarningEl = document.getElementById('storeInactiveWarning');
            ...
            if (storeWarningEl) storeWarningEl.style.display = isStoreInactive ? 'flex' : 'none';
