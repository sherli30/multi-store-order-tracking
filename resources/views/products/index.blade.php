...
@section('styles')
    ...
    display: grid;
    /* Mengatur agar ada 5 kolom sejajar dengan lebar yang sama */
    ...
    .filter-grid {
    /* Di tablet menjadi 3 kolom atau 2 kolom */
    ...
    .product-meta { font-size: 10.5px; color: var(--text-4); margin-top: 4px; line-height: 1.4; display:flex;
    ...
    .price-value { font-size: 13px; font-weight: 800; color: var(--text-1); font-variant-numeric: tabular-nums; white-space:
    nowrap; }
    ...
    /* ── Category/Store inactive indicators ─────────── */
    .pill-inactive { opacity: 0.6; border-style: dashed; }
    .inactive-dot { color: var(--amber); font-size: 9px; }
    .store-badge-inactive { opacity: 0.6; border-style: dashed; }

    /* ── Stock Modal ─────────────────────────────────── */
    .stock-modal-box { width: 420px; }
    .stock-modal-icon-green { background: var(--green-dim); }
    .stock-modal-icon-green svg { color: var(--green); }
    .stock-modal-icon-red { background: var(--red-dim); }
    .stock-modal-icon-red svg { color: var(--red); }
    .stock-modal-info {
    background: var(--surface); border: 1px solid var(--border); border-radius: 10px;
    padding: 12px 16px; margin-bottom: 18px; display: flex; align-items: center;
    justify-content: space-between; gap: 12px;
    }
    .stock-modal-info-label { font-size: 11.5px; color: var(--text-3); font-weight: 600; text-transform: uppercase;
    ...
    .stock-modal-label { font-size: 11.5px; font-weight: 700; color: var(--text-3); text-transform: uppercase;
    letter-spacing: 0.05em; margin-bottom: 7px; display: block; }
    ...
    .btn-success { padding: 9px 18px; border: none; border-radius: 8px; font-family: var(--font); font-size: 13px;
    font-weight: 600; background: var(--green); color: #fff; cursor: pointer; transition: all 0.15s;
...
@section('content')
    ...
    {{-- Flash & Error Alerts Handled by Global Toast --}}
    ...
    <div class="filter-card">
        <div class="filter-card-top">
            ...
            @if (request()->hasAny(['store_id', 'category_id', 'status', 'stock', 'price']))
            ...
            <div class="filter-grid">
                ...
                <div class="form-group">
                    ...
                    <select name="price" class="form-input">
                        ...
                        <option value="low" {{ request('price') == 'low' ? 'selected' : '' }}>Murah (&lt; 50rb)
                        </option>
                        <option value="medium" {{ request('price') == 'medium' ? 'selected' : '' }}>Sedang (50rb–200rb)
                        </option>
                        <option value="high" {{ request('price') == 'high' ? 'selected' : '' }}>Mahal (&gt; 200rb)
                        </option>
                    </select>
                </div>
    ...
    <div class="modal-overlay" id="deleteModal">
        <div class="modal-box">
            ...
            <div class="modal-desc" id="modal-desc">
                Produk ini akan dihapus. Tindakan ini tidak dapat dibatalkan.
            </div>
...
@push('scripts')
    ...
    <script>
        ...
        // Flash Alert logic handled globally
        // Check layouts/app.blade.php for global toast integration
        ...
        function openDeleteModal(productSlug, productName) {
            ...
            document.getElementById('deleteForm').action = '/products/' + productSlug;
        ...
        $(document).ready(function() {
            ...
            $('#filter_store_id').on('change', function() {
                ...
                if (!storeId) {
                    // Reset to show all categories if needed, or just leave as is
                    // But usually, if no store is selected, we should show all categories.
                    // For now, let's just clear and show "Semua Kategori"
                ...
                $.get(url, function(data) {
                    ...
                    categorySelect.html('<option value="">Gagal memuat</option>').prop('disabled',
                        false);
                });
            });
